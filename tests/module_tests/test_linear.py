...
def _test_linear_forward(input_shape, output_channels):
    in_channels = input_shape[1]
    input = np.random.random(input_shape).astype(np.float32) * 20
    original_input = input.copy()
    layer = Linear(in_channels, output_channels)

    torch_layer = nn.Linear(in_channels, output_channels, bias=True)
    utils.assign_linear_weights(layer, torch_layer)
...
def _test_linear_backward(input_shape, output_channels):
    in_channels = input_shape[1]
    input = np.random.random(input_shape).astype(np.float32) * 20
    layer = Linear(in_channels, output_channels)
